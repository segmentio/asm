...
func main() {
	...
	// Broadcast the byte to a GP64 and YMM register.
	tmp := GP64()
	for i := 3; i <= 5; i++ {
		MOVQ(needle, tmp)
		SHLQ(U8(1 << i), tmp)
	...
	Label("impl")
	VariableLengthBytes([]Register{haystack}, length, func (inputs []Register, memory ...Memory) {
		haystack := inputs[0]
		...
		case 16:
			for i := range memory {
				PCMPEQB(needleVec.(Vec).AsX(), regs[i])
			}
			result := reduce(regs, binary(POR))
			PTEST(result, zero.(Vec).AsX())
			JCC(LabelRef("found"))
		case 32:
			for i := range memory {
